...
export default function Hero() {
  ...
  useEffect(() => {
    const loop = async () => {
      while (true) {
        await shimmerControls.start({ opacity: [0.3, 1, 0.3], transition: { duration: 2.5, ease: 'easeInOut' } })
    ...
    >
      ...
      <div className="relative max-w-6xl mx-auto px-6 py-20 w-full">
        <div className="grid md:grid-cols-2 gap-12 items-center">
          ...
          {/* ── Left ── */}
          <motion.div variants={fadeLeft} initial="hidden" animate="visible" className="flex flex-col gap-6">
            {/* Availability badge */}
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border
                            border-brand-300/40 dark:border-brand-700/40
                            bg-brand-50 dark:bg-brand-900/20 w-fit">
              <span className="w-2 h-2 rounded-full bg-green-400 animate-pulse" />
              <span className="text-xs font-medium text-brand-700 dark:text-brand-300">Available for opportunities</span>
            </div>

            <div>
              <p className="text-sm font-mono text-brand-500 mb-2 tracking-widest uppercase">Hello, I'm</p>
            ...
            <p className="text-base text-gray-500 dark:text-gray-400 max-w-md leading-relaxed">
              "Building reliable and user-focused web solutions with modern technologies."
            </p>
            ...
            <div className="flex items-center gap-4 pt-2">
              <a href="https://github.com/martinmwagambo" target="_blank" rel="noopener noreferrer" aria-label="GitHub"
              ...
              <a href="https://www.linkedin.com/in/martin-mwagambo" target="_blank" rel="noopener noreferrer" aria-label="LinkedIn"
                className="p-2.5 rounded-xl border border-gray-200 dark:border-dark-border text-gray-500 dark:text-gray-400
                           hover:text-brand-500 hover:border-brand-400 transition-all duration-200 hover:scale-110">
                <FiLinkedin size={18} />
              </a>
